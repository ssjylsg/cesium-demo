...
var DrawHelper = (function() {
    ...
    function _(map) {
        this.map = map;
        this._scene = map.viewer.scene;
        this._tooltip = createTooltip(map.viewer.container);
        this._surfaces = [];
        this._drawing = true;
    ...
    _.prototype.startDrawingMarker = function(options) {
        ...
        mouseHandler.setInputAction(function(movement) {
            ...
            if (position != null) {
                ...
                if (cartesian) {
                    tooltip.showAt(position, "<p>右键新增标注. 当前位置: </p>" + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
                } else {
    ...
    _.DrawHelperWidget = (function() {
        ...
        function _(drawHelper, options) {
            ...
            var drawOptions = {
                ...
                polygonIcon: host + "img/glyphicons_096_vector_path_polygon.png",
                circleIcon: host + "img/glyphicons_095_vector_path_circle.png",
                extentIcon: host + "img/glyphicons_094_vector_path_square.png",
                clearIcon: host + "img/glyphicons_067_cleaning.png",
    ...
    function enhanceWithListeners(element) {
        return NPMap3D.Util.enhanceWithListeners(element);
    }
    ...
    _.prototype.start = function() {
        ...
        toolbar.addListener('markerCreated', function(event) {
            ...
            //     loggingMessage('Marker edited, ', 'Marker', event.positions);
            // });
        });
        toolbar.addListener('polylineCreated', function(event) {
            if (drawHelper._drawing === true) {
                loggingMessage('Polyline created with ', 'Polyline', event.positions);
            }
            ...
            //     loggingMessage('Polyline', 'Polyline', event.positions);
            // });

        });
        toolbar.addListener('polygonCreated', function(event) {
            if (drawHelper._drawing === true) {
                loggingMessage('Polygon created with ', 'Polygon', event.positions);
            }
            ...
            //     loggingMessage('Polygon', 'Polygon', event.positions);
            // });

        });
        toolbar.addListener('circleCreated', function(event) {
            loggingMessage('Circle', 'Circle', {
                center: event.center,
                radius: event.radius
            });
        ...
        toolbar.addListener('extentCreated', function(event) {
            ...
            loggingMessage('Extent', 'Extent', extent);
        ...
        function loggingMessage(message, type, options) {
            ...
            switch (type) {
                case "Marker":
                    ...
                    break;
                case "Polyline":
                    p = new NPMap3D.Overlay.Polyline({
                        position: convertPosition(options)
                    });
                    break;
                case "Polygon":
                    p = new NPMap3D.Overlay.Polygon({
                        position: convertPosition(options)
                    });
                    break;
                case "Circle":
                    p = new NPMap3D.Overlay.Ellipse({
                        position: convertPosition(options.center),
            ...
            if (p) {
                helper.currentOverlay = p;
                helper.map.addOverlay(p);
            }
        }
    };
    ...
    _.prototype.measureDistance = function(option) {
        ...
        this.startDrawingPolyline({
            callback: function(positions) {
                ...
                var length = 0;
                for (var i = points.length - 1; i >= 0; i--) {
                    length += NPMap3D.distinct(points[i], points[i - 1]);
                    if (i == 1) {
                        break;
                    }
                }
                ...
                var entity0 = new Cesium.Entity({
                    position: Cesium.Cartesian3.fromDegrees((xPoints[0] + xPoints[xPoints.length - 1]) / 2.0, yPoints[yPoints.length - 1]),
                    label: {
                        text: '总长' + length,
                        font: '12px Helvetica',
                ...
                that.map.viewer.entities.add(entity0);
                var id = NPMap3D.generateUUID();
                ...
                that.map.viewer.entities.add(entity1);

                var scene = that.map.viewer.scene;
                ...
                (function(entity, entity0, entity1, id) {
                    var handler = new Cesium.ScreenSpaceEventHandler(that.map.viewer.scene.canvas);
                    handler.setInputAction(function(c) {
                        var f = that.map.viewer.scene.pick(c.position);
                        if (f && f.id && f.id._id === id) {
                            that.map.viewer.entities.remove(entity);
                            that.map.viewer.entities.remove(entity0);
                            that.map.viewer.entities.remove(entity1);
    ...
    _.prototype.measureArea = function(option) {
        ...
        this.startDrawingPolygon({
            callback: function(positions) {
                var points = convertPosition(positions);
                var text = NPMap3D.Util.getGeodesicArea(points).toFixed(2);
                var xPoints = [];
                var yPoints = [];
                for (var i = points.length - 1; i >= 0; i--) {
                    xPoints.push(points[i].x);
                ...
                that.map.viewer.entities.add(entity2);
                ...
                (function(entity2, entity0, entity1, id) {
                    ...
                    handler.setInputAction(function(c) {
                        var f = that.map.viewer.scene.pick(c.position);
                        if (f && f.id && f.id._id === id) {
                            that.map.viewer.entities.remove(entity2);
                            that.map.viewer.entities.remove(entity0);
                            that.map.viewer.entities.remove(entity1);
                            _billboards.removeAll();
                            handler.destroy();
